...
SELECT VR.COD_PACIENTE,
      ...
      LISTAGG(TC.DSC_TIPO_VACINA,','||CHR(10)) WITHIN GROUP(ORDER BY TC.DSC_TIPO_VACINA) DSC_TIPO_VACINA, --ex1
      RTRIM(XMLAGG(XMLELEMENT(E,TC.DSC_TIPO_VACINA,','||CHR(10)).EXTRACT('//text()') ORDER BY TC.DSC_TIPO_VACINA).GetClobVal(),',') DSC_TIPO_VACINA  --ex2    
  FROM VACINA_RECEPCAO VR
